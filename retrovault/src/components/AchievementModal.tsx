...
function getProgress(achievement: Achievement, ctx?: AchievementContext): { current: number; target: number; label: string } | null {
  ...
  const progressMap: Record<string, { current: number; target: number; label: string }> = {
    ...
    "sys_uptime7":  { current: (ctx as any).uptimeDays || 0,       target: 7,   label: "days running" },
    "sys_uptime30": { current: (ctx as any).uptimeDays || 0,       target: 30,  label: "days running" },
    "sys_uptime90": { current: (ctx as any).uptimeDays || 0,       target: 90,  label: "days running" },
    "sys_uptime365":{ current: (ctx as any).uptimeDays || 0,       target: 365, label: "days running" },
    "sys_snapshot7":{ current: (ctx as any).valueHistoryDays || 0, target: 7,   label: "days of history" },
    "sys_api_3":    { current: (ctx as any).apiKeysCreated || 0,   target: 3,   label: "API keys" },
    "sys_bug3":     { current: (ctx as any).bugReportsFiled || 0,  target: 3,   label: "bug reports" },
    "sys_scraper5": { current: (ctx as any).scraperRuns || 0,      target: 5,   label: "scraper runs" },
    "sys_scraper20":{ current: (ctx as any).scraperRuns || 0,      target: 20,  label: "scraper runs" },
